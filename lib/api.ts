...
interface OrderProduct {
  ...
  sale: number
  totalSum: string
  status: "new" | "accepted" | "cancelled" | "sold" | "loaded" | "received"
  type: "standart" | "nonstandart"
  seller: {
    id: string
    createdAt: string
  ...
  sps: {
    status: "active" | "pending" | "defected"
    sp: {
      id: string
      product: {
        id: string
...
interface Selling {
  id: string
  isAccepted: boolean | string | null
  createdAt: string
  storehouse: {
    id: string
    createdAt: string
...
class ApiService {
  ...
  async getSellings(
  ...
  ): Promise<ApiResponse<SellingListResponse>> {
    const searchParams = new URLSearchParams()

    if (params.pageNumber) searchParams.append("pageNumber", params.pageNumber.toString())
    if (params.pageSize) searchParams.append("pageSize", params.pageSize.toString())
    if (params.search) searchParams.append("search", params.search)
    if (params.isAccepted !== undefined) searchParams.append("isAccepted", params.isAccepted)
    if (params.pagination !== undefined) {
      searchParams.append("pagination", params.pagination.toString())
    } else {
      searchParams.append("pagination", "true")
    ...
    return this.request<SellingListResponse>(`/selling/many?${searchParams}`)
  }

  async getSellingById(id: string): Promise<ApiResponse<Selling>> {
    return this.request<Selling>(`/selling/one?id=${id}`)
  }
  ...
  async updateSelling(
    ...
    return this.request(`/selling/one?id=${id}`, {
      method: "PATCH",
      body: JSON.stringify(data),
    })
  }
  ...
  async getPurchases(
    ...
    return this.request<PurchaseListResponse>(`/purchase/many?${searchParams}`)
  }

  async getPurchaseById(id: string): Promise<ApiResponse<Purchase>> {
    return this.request<Purchase>(`/purchase/one?id=${id}`)
  }
  ...
  async updatePurchase(
    ...
    return this.request(`/purchase/one?id=${id}`, {
      method: "PATCH",
      body: JSON.stringify(data),
    })
  }
  ...
  async createPurchase(data: {
    ...
    return this.request("/purchase/one", {
      method: "POST",
      body: JSON.stringify(data),
    })
  }
  ...
  async getOrderProducts(
  ...
  ): Promise<ApiResponse<OrderProductListResponse>> {
    const searchParams = new URLSearchParams()
    ...
    // Hozirgi userning ID sini providerId sifatida qo'shamiz
    if (typeof window !== "undefined") {
      const user = JSON.parse(localStorage.getItem("user") || "{}")
      if (user.id) {
        searchParams.append("providerId", user.id)
      }
    }

    return this.request<OrderProductListResponse>(`/order-product/many?${searchParams}`)
  }

  async getOrderProductById(id: string): Promise<ApiResponse<OrderProduct>> {
    return this.request<OrderProduct>(`/order-product/one?id=${id}`)
  }

  async updateOrderProduct(
    id: string,
    data: {
      status: "accepted" | "cancelled" | "sold" | "loaded" | "received"
    },
  ): Promise<ApiResponse> {
    return this.request(`/order-product/one?id=${id}`, {
      method: "PATCH",
      body: JSON.stringify(data),
    })
  }
